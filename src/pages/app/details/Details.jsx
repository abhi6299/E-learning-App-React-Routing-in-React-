...
function Details() {
  ...
  console.log(params);
  const course = coursesData.find((course)=>course.id==params.courseId);
  console.log(course);
    ...
    <div className={style.courses_container}>
      <div className={style.card_container}>
        ...
        <div className={style.card_content}>
          ...
          <h1 className={style.card_title}>{course.tite}</h1>
          <p className={style.card_description}>{course.description}</p>
        </div>
      </div>
      {/* Task4: create Link to the Learn Page */}
      <button className={style.button}><Link to={`/learn/${params.courseId}`}>Start Learning</Link></button>
    </div>
  );
}
